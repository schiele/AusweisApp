...
function(GET_FILE_EXTENSIONS _result)
	...
	if(_PARAM_PATH)
		foreach(path ${_PARAM_PATH})
			...
			endforeach()
		endforeach()
		set(FILE_EXTENSIONS ${tmpExt})
	endif()

	set(${_result} ${FILE_EXTENSIONS} PARENT_SCOPE)
endfunction()

function(GET_PLATFORM_LIBRARY_FILES _out)
	set(options)
	set(oneValueArgs)
	set(multiValueArgs EXCLUDE)
	cmake_parse_arguments(_PARAM "${options}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})

	GET_FILE_EXTENSIONS(FILE_EXTENSIONS ${ARGN})
	...
	set(${_out} ${FILES} PARENT_SCOPE)
endfunction()

function(ADD_PLATFORM_LIBRARY _name)
	GET_PLATFORM_LIBRARY_FILES(FILES ${ARGN})
...
function(version_to_hex _out _version)
	if(_version MATCHES "^([0-9]+)\\.([0-9]+)$")
		string(APPEND _version ".0")
	endif()
	if(NOT _version MATCHES "([0-9]+)\\.([0-9]+)\\.([0-9]+)")
		message(FATAL_ERROR "Version is invalid: '${_version}'")
	endif()

	math(EXPR result "((${CMAKE_MATCH_1} << 16) | (${CMAKE_MATCH_2} << 8) | ${CMAKE_MATCH_3})" OUTPUT_FORMAT HEXADECIMAL)
	string(LENGTH "${result}" len)
	if(len EQUAL 7)
		string(REGEX REPLACE "^0x" "0x0" result "${result}")
	endif()
	set(${_out} "${result}" PARENT_SCOPE)
endfunction()
