	...
	: public QObject
{
	...
	private Q_SLOTS:
		...
		void test_onAppDownloadFinished_data()
		{
			...
			QTest::addColumn<bool>("errorSignal");
			QTest::addColumn<bool>("errorMessage");

			QTest::addRow("Data corrupted") << GlobalStatus::Code::Downloader_Data_Corrupted << true << true;
			QTest::addRow("Aborted") << GlobalStatus::Code::Downloader_Aborted << true << false;
			QTest::addRow("Execution failed") << GlobalStatus::Code::Update_Execution_Failed << true << true;
			QTest::addRow("Unknown") << GlobalStatus::Code::Unknown_Error << true << true;
#if !defined(Q_OS_WIN) // Avoid to launch msiexec.exe in Unit Tests
			QTest::addRow("No error -> Execution failed") << GlobalStatus::Code::No_Error << true << true;
#endif
		}
		...
		void test_onAppDownloadFinished()
		{
			...
			QCOMPARE(failedSpy.count(), errorSignal ? 1 : 0);

			if (errorSignal)
			{
				QVERIFY(failedSpy.takeFirst().at(0).toString().isEmpty() != errorMessage);
			}
		}
