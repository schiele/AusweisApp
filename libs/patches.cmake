...
function(FILTER_PARAMETER _list)
	set(tmpList ${${_list}})
	foreach(entry IN LISTS tmpList)
		if(NOT _PARAM_${entry})
			list(REMOVE_ITEM tmpList "${entry}")
		endif()
	endforeach()

	list(LENGTH tmpList tmpListLength)
	if(NOT tmpListLength EQUAL 0)
		set(${_list} ${tmpList} PARENT_SCOPE)
	endif()
endfunction()

list(APPEND COMPONENTS llhttp openssl qt)

GET_PARAMS(parameter)
set(options apply generate ${COMPONENTS})
...
if(NOT CMAKE_SCRIPT_MODE_FILE OR _PARAM_UNPARSED_ARGUMENTS)
	...
	message(FATAL_ERROR "Usage: cmake -P libs/patches.cmake apply|generate [${COMPONENTS}]")
endif()

FILTER_PARAMETER(COMPONENTS)
...
cmake_path(NORMAL_PATH REPOSITORY_DIR OUTPUT_VARIABLE REPOSITORY_DIR)
...
function(get_patches _out _prefixes)
	foreach(prefix ${_prefixes})
		file(GLOB prefixedPatches "${patch_dir}/${prefix}*.patch")
		list(APPEND PATCHES ${prefixedPatches})
	endforeach()
...
function(git_clone prefix)
	...
	message(STATUS "Use repository path: ${REPOSITORY_PATH}")
	if(NOT EXISTS "${REPOSITORY_PATH}/.git")
		...
		elseif(prefix MATCHES "qt")
			set(QT_CGIT_BASE_URL https://code.qt.io/cgit/qt)
			set(QT_GIT_HOOKS_URL "${QT_CGIT_BASE_URL}/qtrepotools.git/plain/git-hooks")
			...
			file(DOWNLOAD "${QT_CGIT_BASE_URL}/qt5.git/plain/.commit-template" "${REPOSITORY_PATH}/.git/.commit-template")
			execute_dir(${prefix} config commit.template ".git/.commit-template")
		endif()
	endif()
endfunction()
...
function(apply_patches _prefixes)
	get_patches(PATCHES "${_prefixes}")
...
function(generate_patches _prefixes)
	get_patches(PATCHES "${_prefixes}")
	if(PATCHES)
		file(REMOVE ${PATCHES})
	endif()
...
if(_PARAM_apply)
	message(STATUS "Apply patches for components: ${COMPONENTS}")
	apply_patches("${COMPONENTS}")
endif()

if(_PARAM_generate)
	message(STATUS "Generate patches for components: ${COMPONENTS}")
	generate_patches("${COMPONENTS}")
endif()
